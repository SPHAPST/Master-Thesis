####### READABILITY METRIC #######
#------ Lines and points overlapped by the icons ------#
#------ Calculate the disturbance factor ------#

####### FOR OPTIMIZATION #######
#------ Define a final function that takes as input x,y coords of the new placed icons 
#------ and returns the readability metric as average for all the features ------#
...
###### Readability metric: Number of line segments overlapped by the icon labels ######

def readability_lines(x_coords, y_coords, g_layer):
    line_layer = project.mapLayersByName('clip_layer_lines')[0]
    ...
    for i_layer in g_layer:
        ...
        for x, y in zip(x_coords, y_coords):
            ...
            count_lines= len(overlapping_line_features)
...
###### Readability metric: Number of points overlapped by the icon labels ########

def readability_points(x_coords, y_coords, g_layer):
    point_layer = project.mapLayersByName('clip_layer_points')[0]
        ...
        for x, y in zip(x_coords, y_coords):
                ...
                for id in ids:
                            ...
                            if point_feature.geometry().intersects(bbox):
                                ...
                                intersection = point_feature.geometry().intersection(bbox_geom)
...
# The function that will be used for optimization

def f1(x_coords, y_coords, g_layer):
        all_line_factors = {}  # Dictionary to store the readability factors of all layers
        all_point_factors = {} 

        # Get the factors for all the layers in the group
        for i_layer in g_layer:
            line_factors = readability_lines(x_coords, y_coords, [i_layer])
            point_factors = readability_points(x_coords, y_coords, [i_layer])

            all_line_factors[i_layer] = line_factors
            all_point_factors[i_layer] = point_factors

        # List to store all disturbance factors
        all_disturbance_factors = []

        # Calculate and normalize disturbance factors for each layer
        for i_layer in g_layer:
            line_factors = all_line_factors[i_layer]
            point_factors = all_point_factors[i_layer]
...
# Call the function for each layer in the group
...
for i_layer in g_layer:
    ...
    factors = f1(x_coords, y_coords, [i_layer])
    print(f"Avg disturbance factor for layer {i_layer.name()}: {factors}"),19
